...
FORCE=0
# dbatools.library layout: all C# sources live under project/ (1110 files).
# This hook came from the dbatools repo, which scans public,private — those
# directories do not exist here, so the inherited value silently scanned nothing
# and wrote an EMPTY baseline. An empty baseline makes the ratchet treat every
# pre-existing clone as new duplication.
ROOT="project"
MIN_TOKENS=50
