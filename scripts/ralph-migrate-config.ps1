<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the config domain (18 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert config commands from PS1 to C# binary cmdlets.
    Each iteration converts ONE command, verifies the build, and commits.

.PARAMETER MaxIterations
    Maximum iterations before stopping (default: 28).
...
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-CONFIG.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-config-complete')
...
Write-Host "  Commands: 18 | Max iterations: $MaxIterations" -ForegroundColor Cyan
