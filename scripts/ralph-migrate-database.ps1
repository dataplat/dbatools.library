<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the database domain (116 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert database commands from PS1 to C# binary cmdlets.
    Each iteration converts ONE command, verifies the build, and commits.

.PARAMETER MaxIterations
    Maximum iterations before stopping (default: 126).
...
[CmdletBinding()]
param(
    [int]$MaxIterations = 126,
    [switch]$DryRun
)

$ErrorActionPreference = 'Stop'
$RepoRoot = 'c:\github\dbatools.library'
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-DATABASE.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-database-complete')
$PromptPath = [IO.Path]::Combine($RepoRoot, 'scripts', 'migrate-prompt.md')
$Domain = 'database'
$DomainUpper = 'DATABASE'
...
Write-Host "  Commands: 116 | Max iterations: $MaxIterations" -ForegroundColor Cyan
