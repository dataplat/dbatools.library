<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the backup domain (20 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert backup commands from PS1 to C# binary cmdlets.
...
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-BACKUP.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-backup-complete')
