<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the dbobject domain (51 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert dbobject commands from PS1 to C# binary cmdlets.
    Each iteration converts ONE command, verifies the build, and commits.

.PARAMETER MaxIterations
    Maximum iterations before stopping (default: 61).
...
[CmdletBinding()]
param(
    [int]$MaxIterations = 61,
    [switch]$DryRun
)

$ErrorActionPreference = 'Stop'
$RepoRoot = 'c:\github\dbatools.library'
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-DBOBJECT.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-dbobject-complete')
$PromptPath = [IO.Path]::Combine($RepoRoot, 'scripts', 'migrate-prompt.md')
$Domain = 'dbobject'
$DomainUpper = 'DBOBJECT'
...
Write-Host "  Commands: 51 | Max iterations: $MaxIterations" -ForegroundColor Cyan
