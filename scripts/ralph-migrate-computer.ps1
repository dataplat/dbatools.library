<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the computer domain (50 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert computer commands from PS1 to C# binary cmdlets.
    Each iteration converts ONE command, verifies the build, and commits.

.PARAMETER MaxIterations
    Maximum iterations before stopping (default: 60).
...
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-COMPUTER.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-computer-complete')
$PromptPath = [IO.Path]::Combine($RepoRoot, 'scripts', 'migrate-prompt.md')
$Domain = 'computer'
$DomainUpper = 'COMPUTER'
...
Write-Host "  Commands: 50 | Max iterations: $MaxIterations" -ForegroundColor Cyan
