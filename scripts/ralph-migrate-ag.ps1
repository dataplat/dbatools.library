<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the ag domain (35 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert ag commands from PS1 to C# binary cmdlets.
    Each iteration converts ONE command, verifies the build, and commits.

.PARAMETER MaxIterations
    Maximum iterations before stopping (default: 45).
...
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-AG.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-ag-complete')
...
Write-Host "  Commands: 35 | Max iterations: $MaxIterations" -ForegroundColor Cyan
