<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the __DOMAIN__ domain (__COUNT__ commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert __DOMAIN__ commands from PS1 to C# binary cmdlets.
    Each iteration converts ONE command, verifies the build, and commits.

.PARAMETER MaxIterations
    Maximum iterations before stopping (default: __MAXITER__).
...
[CmdletBinding()]
param(
    [int]$MaxIterations = __MAXITER__,
    [switch]$DryRun
)

$ErrorActionPreference = 'Stop'
$RepoRoot = 'c:\github\dbatools.library'
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-__UPPER__.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-__DOMAIN__-complete')
$PromptPath = [IO.Path]::Combine($RepoRoot, 'scripts', 'migrate-prompt.md')
$Domain = '__DOMAIN__'
$DomainUpper = '__UPPER__'
$Dependencies = '__DEPS__'
...
Write-Host "  Commands: __COUNT__ | Max iterations: $MaxIterations" -ForegroundColor Cyan
