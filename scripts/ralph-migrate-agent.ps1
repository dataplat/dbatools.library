<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the agent domain (51 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert agent commands from PS1 to C# binary cmdlets.
...
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-AGENT.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-agent-complete')
