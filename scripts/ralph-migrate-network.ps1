<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the network domain (31 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert network commands from PS1 to C# binary cmdlets.
    Each iteration converts ONE command, verifies the build, and commits.

.PARAMETER MaxIterations
    Maximum iterations before stopping (default: 41).
...
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-NETWORK.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-network-complete')
...
Write-Host "  Commands: 31 | Max iterations: $MaxIterations" -ForegroundColor Cyan
