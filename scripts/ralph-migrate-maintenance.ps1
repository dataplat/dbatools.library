<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the maintenance domain (29 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert maintenance commands from PS1 to C# binary cmdlets.
    Each iteration converts ONE command, verifies the build, and commits.

.PARAMETER MaxIterations
    Maximum iterations before stopping (default: 39).
...
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-MAINTENANCE.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-maintenance-complete')
$PromptPath = [IO.Path]::Combine($RepoRoot, 'scripts', 'migrate-prompt.md')
$Domain = 'maintenance'
$DomainUpper = 'MAINTENANCE'
...
Write-Host "  Commands: 29 | Max iterations: $MaxIterations" -ForegroundColor Cyan
