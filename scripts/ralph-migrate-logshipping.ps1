<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the logshipping domain (14 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert logshipping commands from PS1 to C# binary cmdlets.
    Each iteration converts ONE command, verifies the build, and commits.

.PARAMETER MaxIterations
    Maximum iterations before stopping (default: 24).
...
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-LOGSHIPPING.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-logshipping-complete')
$PromptPath = [IO.Path]::Combine($RepoRoot, 'scripts', 'migrate-prompt.md')
$Domain = 'logshipping'
$DomainUpper = 'LOGSHIPPING'
...
Write-Host "  Commands: 14 | Max iterations: $MaxIterations" -ForegroundColor Cyan
