<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the resourcegovernor domain (19 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert resourcegovernor commands from PS1 to C# binary cmdlets.
    Each iteration converts ONE command, verifies the build, and commits.

.PARAMETER MaxIterations
    Maximum iterations before stopping (default: 29).
...
[CmdletBinding()]
param(
    [int]$MaxIterations = 29,
    [switch]$DryRun
)

$ErrorActionPreference = 'Stop'
$RepoRoot = 'c:\github\dbatools.library'
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-RESOURCEGOVERNOR.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-resourcegovernor-complete')
$PromptPath = [IO.Path]::Combine($RepoRoot, 'scripts', 'migrate-prompt.md')
$Domain = 'resourcegovernor'
$DomainUpper = 'RESOURCEGOVERNOR'
...
Write-Host "  Commands: 19 | Max iterations: $MaxIterations" -ForegroundColor Cyan
