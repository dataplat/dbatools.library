<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the perfcounter domain (15 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert perfcounter commands from PS1 to C# binary cmdlets.
...
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-PERFCOUNTER.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-perfcounter-complete')
$PromptPath = [IO.Path]::Combine($RepoRoot, 'scripts', 'migrate-prompt.md')
$Domain = 'perfcounter'
$DomainUpper = 'PERFCOUNTER'
...
Write-Host "  Commands: 15 | Max iterations: $MaxIterations" -ForegroundColor Cyan
