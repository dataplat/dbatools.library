<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the instance domain (95 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert instance commands from PS1 to C# binary cmdlets.
    Each iteration converts ONE command, verifies the build, and commits.

.PARAMETER MaxIterations
    Maximum iterations before stopping (default: 105).
...
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-INSTANCE.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-instance-complete')
$PromptPath = [IO.Path]::Combine($RepoRoot, 'scripts', 'migrate-prompt.md')
$Domain = 'instance'
$DomainUpper = 'INSTANCE'
...
Write-Host "  Commands: 95 | Max iterations: $MaxIterations" -ForegroundColor Cyan
