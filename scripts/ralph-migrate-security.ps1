<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the security domain (71 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert security commands from PS1 to C# binary cmdlets.
    Each iteration converts ONE command, verifies the build, and commits.

.PARAMETER MaxIterations
    Maximum iterations before stopping (default: 81).
...
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-SECURITY.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-security-complete')
$PromptPath = [IO.Path]::Combine($RepoRoot, 'scripts', 'migrate-prompt.md')
$Domain = 'security'
$DomainUpper = 'SECURITY'
...
Write-Host "  Commands: 71 | Max iterations: $MaxIterations" -ForegroundColor Cyan
