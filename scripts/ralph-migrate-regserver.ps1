<#
.SYNOPSIS
    Ralph Wiggum migration orchestrator for the regserver domain (12 commands).

.DESCRIPTION
    Iteratively invokes Claude Code to convert regserver commands from PS1 to C# binary cmdlets.
    Each iteration converts ONE command, verifies the build, and commits.

.PARAMETER MaxIterations
    Maximum iterations before stopping (default: 22).
...
$TrackerPath = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', 'TRACKER-MIGRATE-REGSERVER.md')
$SignalFile = [IO.Path]::Combine($RepoRoot, 'docs', 'plan', '.migrate-regserver-complete')
$PromptPath = [IO.Path]::Combine($RepoRoot, 'scripts', 'migrate-prompt.md')
$Domain = 'regserver'
$DomainUpper = 'REGSERVER'
...
Write-Host "  Commands: 12 | Max iterations: $MaxIterations" -ForegroundColor Cyan
