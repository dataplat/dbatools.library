...
/// <summary>
/// Retrieves Policy-Based Management policy categories from one or more SQL Server instances.
/// </summary>
/// <remarks>
/// <para>
/// The workflow remains a module-scoped PowerShell compatibility hop so that the PBM library load, the
/// store enumeration, the category retrieval, the added note properties, the default view, and dbatools
...
/// earlier-output-before-later-throw exposure and buffered InvokeScoped is correct. Both switch
/// parameters are carried as plain (untyped) values, because a switch in the inner CmdletBinding
/// scriptblock is excluded from positional binding.
/// </para>
/// </remarks>
[Cmdlet(VerbsCommon.Get, "DbaPbmCategory")]
public sealed class GetDbaPbmCategoryCommand : DbaBaseCmdlet
{
    ...
    /// <summary>Filters results to policy categories with these names.</summary>
    ...
    /// <summary>Excludes system (built-in) policy categories from the results.</summary>
    [Parameter]
    public SwitchParameter ExcludeSystemObject { get; set; }
    ...
    /// <summary>Retrieves policy categories for one pipeline record.</summary>
    protected override void ProcessRecord()
    {
        ...
        foreach (PSObject? item in NestedCommand.InvokeScoped(this, ProcessScript,
            SqlInstance, SqlCredential, Category, InputObject,
            ExcludeSystemObject.ToBool(), EnableException.ToBool(),
            BoundCommonParameter("Verbose"), BoundCommonParameter("Debug")))
        {
            if (item?.BaseObject is ErrorRecord nestedError)
            {
...
param($SqlInstance, $SqlCredential, $Category, $InputObject, $ExcludeSystemObject, $EnableException, $__boundVerbose, $__boundDebug)
$__commonParameters = @{}
if ($null -ne $__boundVerbose) { $__commonParameters.Verbose = [bool]$__boundVerbose }
if ($null -ne $__boundDebug -and $PSVersionTable.PSVersion.Major -lt 7) { $__commonParameters.Debug = [bool]$__boundDebug }
$__dbatoolsModule = Get-Module -Name dbatools | Where-Object ModuleType -eq "Script" | Select-Object -First 1
& $__dbatoolsModule {
    [CmdletBinding()]
    param([Dataplat.Dbatools.Parameter.DbaInstanceParameter[]]$SqlInstance, [System.Management.Automation.PSCredential]$SqlCredential, [string[]]$Category, [psobject[]]$InputObject, $ExcludeSystemObject, $EnableException, $__boundVerbose, $__boundDebug)
    if ($null -ne $__boundDebug -and $PSVersionTable.PSVersion.Major -ge 7) { $DebugPreference = $(if ($__boundDebug) { "Continue" } else { "SilentlyContinue" }) }
        ...
        foreach ($store in $InputObject) {
            ...
            foreach ($current in $all) {
                Write-Message -Level Verbose -Message "Processing $current" -FunctionName Get-DbaPbmCategory -ModuleName "dbatools"
...
} $SqlInstance $SqlCredential $Category $InputObject $ExcludeSystemObject $EnableException $__boundVerbose $__boundDebug @__commonParameters 3>&1 2>&1
