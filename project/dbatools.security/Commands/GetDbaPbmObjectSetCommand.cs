...
/// <summary>
/// Retrieves Policy-Based Management policy object sets from one or more SQL Server instances.
/// </summary>
/// <remarks>
/// <para>
/// The workflow remains a module-scoped PowerShell compatibility hop so that the PBM library load, the
/// store enumeration, the object set retrieval, the added note properties, the default view, and dbatools
...
/// </para>
/// </remarks>
[Cmdlet(VerbsCommon.Get, "DbaPbmObjectSet")]
public sealed class GetDbaPbmObjectSetCommand : DbaBaseCmdlet
{
    ...
    /// <summary>Filters results to policy object sets with these names.</summary>
    ...
    /// <summary>Includes system (built-in) policy object sets from the results.</summary>
    [Parameter]
    ...
    /// <summary>Retrieves policy object sets for one pipeline record.</summary>
    protected override void ProcessRecord()
    {
        ...
        foreach (PSObject? item in NestedCommand.InvokeScoped(this, ProcessScript,
            SqlInstance, SqlCredential, ObjectSet, InputObject,
...
param($SqlInstance, $SqlCredential, $ObjectSet, $InputObject, $IncludeSystemObject, $EnableException, $__boundVerbose, $__boundDebug)
$__commonParameters = @{}
if ($null -ne $__boundVerbose) { $__commonParameters.Verbose = [bool]$__boundVerbose }
if ($null -ne $__boundDebug -and $PSVersionTable.PSVersion.Major -lt 7) { $__commonParameters.Debug = [bool]$__boundDebug }
$__dbatoolsModule = Get-Module -Name dbatools | Where-Object ModuleType -eq "Script" | Select-Object -First 1
& $__dbatoolsModule {
    [CmdletBinding()]
    param([Dataplat.Dbatools.Parameter.DbaInstanceParameter[]]$SqlInstance, [System.Management.Automation.PSCredential]$SqlCredential, [string[]]$ObjectSet, [psobject[]]$InputObject, $IncludeSystemObject, $EnableException, $__boundVerbose, $__boundDebug)
    if ($null -ne $__boundDebug -and $PSVersionTable.PSVersion.Major -ge 7) { $DebugPreference = $(if ($__boundDebug) { "Continue" } else { "SilentlyContinue" }) }
...
} $SqlInstance $SqlCredential $ObjectSet $InputObject $IncludeSystemObject $EnableException $__boundVerbose $__boundDebug @__commonParameters 3>&1 2>&1
