...
/// <summary>
/// Enables the HideInstance flag for SQL Server instances. Port of
/// public/Enable-DbaHideInstance.ps1 (W3-011), the sibling of W3-007 Disable-DbaHideInstance
/// (same shape; the remote scriptblock sets HideInstance=$true instead of $false and the
/// Invoke-Command2 result is piped through Select-Object to strip remoting properties).
/// Pure per-record process command with no begin/end blocks, so the whole process body runs as
/// one streaming hop. DEF-001 cond1+cond2: the process foreach EMITS a [PSCustomObject] per
/// instance (returned by the Invoke-Command2 remote scriptblock) AND has reachable
/// Stop-Function -Continue at Resolve-DbaNetworkName / Invoke-ManagedComputerCommand / the
/// REGROOT lookup / Invoke-Command2, so the hop STREAMS via InvokeScopedStreaming - a buffered
/// hop would lose an earlier instance's emit when a later instance throws under
/// -EnableException. The source's SqlInstance default ($env:COMPUTERNAME) is applied in
/// ProcessRecord when the parameter is unbound (the GetDbaClientAlias pattern). Substitutions
/// only: $PScmdlet.ShouldProcess -> $__realCmdlet.ShouldProcess (ConfirmImpact LOW mirrored) and
/// explicit -FunctionName Enable-DbaHideInstance on every Stop-Function (W1-090); the body is
/// otherwise verbatim. Surface pinned by migration/baselines/Enable-DbaHideInstance.json.
/// </summary>
[Cmdlet(VerbsLifecycle.Enable, "DbaHideInstance", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Low, DefaultParameterSetName = "Default")]
    ...
    foreach ($instance in $SqlInstance) {
        ...
        try {
            ...
            try {
                ...
                Stop-Function -Message "Issue resolving $instance" -Target $instance -Category InvalidArgument -Continue -FunctionName Enable-DbaHideInstance
            }
        }
        ...
        try {
            ...
            Stop-Function -Message "Failed to access $instance" -Target $instance -Continue -ErrorRecord $_ -FunctionName Enable-DbaHideInstance
        }
        ...
        if ([System.String]::IsNullOrEmpty($regRoot)) {
            ...
            if (![System.String]::IsNullOrEmpty($regRoot)) {
                ...
                Stop-Function -Message "Can't find instance $vsname on $instance." -Continue -Category ObjectNotFound -Target $instance -FunctionName Enable-DbaHideInstance
            }
        }
