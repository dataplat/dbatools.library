...
/// <summary>
/// Shows a WPF tree-view dialog for browsing a remote instance's file system. Port of
/// public/Show-DbaInstanceFileSystem.ps1 (W3-100). Begin + per-record + end hops.
/// CLASSIFICATION TABLE (scalar SqlInstance IS VFP; promoted question answered): the
/// source's begin{} defines three helper functions and three icon variables in the
/// FUNCTION scope that process reads via the scope chain - the hop's function scope
/// dies per record, so the port RE-DECLARES the helpers and RECOMPUTES the icons
/// inside the process hop (Convert-b64toimg is a pure function of embedded base64
/// constants, so per-record recomputation is observation-identical). The XAML controls
/// ($treeview/$textbox/$okbutton/...) are planted via Set-Variable -Scope Script - in
/// the source that is the MODULE scope, and the hop's Script scope IS the same module
/// scope, so their cross-record AND cross-CALL persistence rides verbatim, including
/// the source's own STALE-TEXTBOX quirk: end{} reads $textbox unconditionally (no
/// interrupt gate), so a call whose process dies before ShowDialog emits the PREVIOUS
/// call's selection (smoke-pinned). EndProcessing is therefore NOT gated on
/// Interrupted. $server is hop-local and read by the re-declared helpers/event
/// handlers via dynamic scoping DURING ShowDialog - all GUI interaction lives inside
/// the one hop invocation. No $Pscmdlet/ShouldProcess in source (plain CmdletBinding,
/// no gates). Begin hop carries $EnableException for Stop-Function's scope-walking
/// [bool]$EnableException = $EnableException default (W3-097 law); hop-frame
/// Stop-Function calls carry -FunctionName (W1-090); the re-declared helpers contain
/// no Stop-Function/Write-Message (no helper-frame concern). Surface pinned by
/// migration/baselines/Show-DbaInstanceFileSystem.json (SqlInstance SCALAR
/// DbaInstanceParameter Mandatory pos0 VFP, SqlCredential pos1, no sets).
/// </summary>
[Cmdlet(VerbsCommon.Show, "DbaInstanceFileSystem")]
public sealed class ShowDbaInstanceFileSystemCommand : DbaBaseCmdlet
{
    ...
    // NOT gated on Interrupted: the source end{} has no Test-FunctionInterrupt, so it
    // runs (and can emit the stale module-scope textbox) even after a begin latch.
    ...
    // PS: the begin Add-Type/latch gate verbatim. The helper functions and icon
    // variables the source also defines in begin are function-scope state the hop
    // cannot persist - they move into the process hop (see class doc).
...
& $__dbatoolsModule {
    ...
    . {
        try {
            ...
            Stop-Function -Message "Windows Presentation Framework required but not installed." -FunctionName Show-DbaInstanceFileSystem
            return
        }
    }
...
""";
    ...
    // PS: the ENTIRE process body VERBATIM per record, with the re-declared begin
    // helpers and recomputed icons inserted AFTER the body's own Test-FunctionInterrupt
    // gate (source order: a latched begin never defines helpers/icons and process
    // returns at the gate - hoisting the gate above the relocated definitions keeps the
    // latched path from touching WPF types that failed to load). Substitutions only:
    // explicit -FunctionName Show-DbaInstanceFileSystem on hop-frame Stop-Function
    // (W1-090). Set-Variable -Scope Script lands in the module scope, exactly like the
    // source.
...
& $__dbatoolsModule {
    ...
    . {
        ...
        try {
            ...
            Stop-Function -Message "Failure" -Category ConnectionError -ErrorRecord $_ -Target $SqlInstance -FunctionName Show-DbaInstanceFileSystem
            return
        }
...
'
        ...
        try {
            ...
            Stop-Function -Message "No access to remote SQL Server files." -Target $SqlInstance -FunctionName Show-DbaInstanceFileSystem
            return
        }
...
""";
    ...
    // PS: the end block VERBATIM - reads the module-scope $textbox unconditionally.
