...
public sealed class GetDbaDbSpaceCommand : DbaInstanceCmdlet
{
    ...
    protected override void ProcessRecord()
    {
        ...
        {
            // PS: ETS-first reads; $server.VersionMajor -lt 9 is TRUE for null.
            object? server = DotAccess(item, "Parent");
            object? versionMajor = DotAccess(server, "VersionMajor");
            bool tooOld = versionMajor is null || LanguagePrimitives.Compare(versionMajor, 9, false, CultureInfo.InvariantCulture) < 0;
            if (tooOld)
            {
    ...
    /// <summary>PS: DBNull maps to int 0, otherwise [Math]::Round(double) (a fault
    /// bubbles to the function's own catch).</summary>
