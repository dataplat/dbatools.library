...
/// <summary>
/// Reports top wait statistics (Paul Randal's query). Port of
/// public/Get-DbaWaitStatistic.ps1 (W1-105). The ENTIRE begin block rides one VERBATIM
/// module hop (the Trim/ToUpperInvariant normalizations, the giant $details/$category
/// bags, the $defaultIgnorable list, the ArrayList ignorable build with its
/// -notcontains/Remove passes, the $ignorableSql/$sql interpolation against $Threshold,
/// and the verbose/debug messages) returning a HASHTABLE (the W1-092 tuple-collapse
/// law); the ValidateScript pair is reproduced by a C# attribute that throws the exact
/// PS validation text; each instance rides one VERBATIM row-loop hop (the
/// ($category).$waitType / ($details).$waitType dynamic reads, the $defaultIgnorable
/// -contains flag, and Select-DefaultView -ExcludeProperty per -IncludeIgnorable).
/// Surface pinned by migration/baselines/Get-DbaWaitStatistic.json.
/// </summary>
[Cmdlet(VerbsCommon.Get, "DbaWaitStatistic")]
public sealed class GetDbaWaitStatisticCommand : DbaInstanceCmdlet
{
    ...
    /// <summary>Cumulative percentage threshold.</summary>
    [Parameter(Position = 2)]
    public int Threshold { get; set; } = 95;

    /// <summary>Includes the normally-ignorable wait types.</summary>
    [Parameter]
    public SwitchParameter IncludeIgnorable { get; set; }

    /// <summary>Extra wait types to exclude.</summary>
    [Parameter(Position = 3)]
    [ValidateWaitType]
    public string[]? ExcludeWaitType { get; set; }

    /// <summary>Wait types to force-include (removed from the ignorable list).</summary>
    [Parameter(Position = 4)]
    [ValidateWaitType]
    public string[]? IncludeWaitType { get; set; }

    // EnableException is inherited from DbaBaseCmdlet - never redeclared.

    /// <summary>PS: [ValidateScript( { -not [string]::IsNullOrWhiteSpace($_) -and
    /// $_.Trim() -match "^[A-Za-z0-9_]+$" })] - same predicate, same failure text.</summary>
    private sealed class ValidateWaitTypeAttribute : ValidateEnumeratedArgumentsAttribute
    {
        protected override void ValidateElement(object element)
        {
            string? text = element as string ?? (element is null ? null : (string)LanguagePrimitives.ConvertTo(element, typeof(string), System.Globalization.CultureInfo.InvariantCulture));
            bool valid = !string.IsNullOrWhiteSpace(text) && System.Text.RegularExpressions.Regex.IsMatch(text!.Trim(), "^[A-Za-z0-9_]+$", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
            if (!valid)
            {
                string script = " -not [string]::IsNullOrWhiteSpace($_) -and $_.Trim() -match \"^[A-Za-z0-9_]+$\" ";
    ...
    private object? _defaultIgnorable;
    ...
    protected override void BeginProcessing()
    {
        ...
        _category = bag is not null ? bag["Category"] : null;
        _defaultIgnorable = bag is not null ? bag["DefaultIgnorable"] : null;
    }
    ...
    protected override void ProcessRecord()
    {
        foreach (DbaInstanceParameter instance in SqlInstance)
        {
            ...
            object excludeColumns = IncludeIgnorable.ToBool()
                ? (object)new string[] { "Notes" }
                : (object)new string[] { "Notes", "Ignorable" };

            try
            {
                foreach (PSObject? item in NestedCommand.InvokeScoped(this, RowProjectionScript, server, _sql, _category, _details, _defaultIgnorable, excludeColumns))
                    WriteObject(item);
            }
            ...
            {
                StatementFault.Surface(this, ex, "Get-DbaWaitStatistic");
            }
        }
    }
    ...
    // PS: the begin block VERBATIM (normalizations, $details/$category bags,
...
""";
    ...
    // -contains flag, the SDV exclude columns).
    private const string RowProjectionScript = """
param($server, $sql, $category, $details, $defaultIgnorable, $excludeColumns)
$__dbatoolsModule = Get-Module -Name dbatools | Where-Object ModuleType -eq "Script" | Select-Object -First 1
& $__dbatoolsModule {
    param($server, $sql, $category, $details, $defaultIgnorable, $excludeColumns)
...
} $server $sql $category $details $defaultIgnorable $excludeColumns 3>&1
""";
}
