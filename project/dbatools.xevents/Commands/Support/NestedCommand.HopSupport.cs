...
internal static partial class NestedCommand
{
    ...
    /// re-emitting cmdlet's type name, so a nested Get-ChildItem failure that the retired
    /// function reported as PathNotFound,...GetChildItemCommand arrives as
    /// PathNotFound,...GetDbaXESessionTemplateCommand. The divergence reaches the caller's
    /// -ErrorVariable as well as the error stream, because WriteError re-stamps the record
    /// in place and the variable holds that same instance.
    ///
