...
/// </summary>
/// <remarks>
/// The trace resolution (Get-DbaTrace), the default-trace guard, the sp_trace_setstatus start call, and
/// the re-query all run the original dbatools PowerShell body VERBATIM inside the dbatools module scope
...
///
/// The three Stop-Function calls are all -Continue and gain -FunctionName Start-DbaTrace (in-hop the
...
/// carry is needed. Each started trace is re-queried and emitted before a later trace's Query may throw
/// under -EnableException, so the process hop uses InvokeScopedStreaming to avoid losing the record of a
/// trace that was actually started (DEF-001). Surface pinned by migration/baselines/Start-DbaTrace.json.
/// </remarks>
[Cmdlet(VerbsLifecycle.Start, "DbaTrace", SupportsShouldProcess = true, ConfirmImpact = ConfirmImpact.Medium)]
public sealed class StartDbaTraceCommand : DbaBaseCmdlet
{
    ...
    /// <summary>The numeric IDs of specific traces to start.</summary>
    ...
    // and -FunctionName Start-DbaTrace on the three Stop-Function sites. EnableException is bound so
...
& $__dbatoolsModule {
    ...
    foreach ($trace in $InputObject) {
        if (-not $trace.id -and -not $trace.Parent) {
            Stop-Function -Message "Input is of the wrong type. Use Get-DbaTrace." -Continue -FunctionName Start-DbaTrace
            return
        }
        ...
        if ($default.id -eq $traceid) {
            Stop-Function -Message "The default trace on $server cannot be started. Use Set-DbaSpConfigure to turn it on." -Continue -FunctionName Start-DbaTrace
        }

        $sql = "EXEC sp_trace_setstatus $traceid, 1"
        if ($__realCmdlet.ShouldProcess($traceid, "Starting the TraceID on $server")) {
            try {
                $server.Query($sql)
                Get-DbaTrace -SqlInstance $server -Id $traceid
            } catch {
                Stop-Function -Message "Failure" -ErrorRecord $_ -Target $server -Continue -FunctionName Start-DbaTrace
                return
            }
        }
    }
